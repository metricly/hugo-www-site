...
In a [previous article](https://www.metricly.com/ebs-burst-balance-aws), I introduced you to EBS Burst Balance and the tools available from Metricly to monitor your EBS Burst Balance, and how to set up alerts when the balance falls below a specified threshold.
...
The approach I'll describe below is one possible solution and depends on your use case and projected growth. A different solution which might warrant an investigation is to use an **IO1** volume type instead. The **IO1** type is a provisioned IOPS SSD solution which supports higher rates of reading and writing.
...
We're now going to connect the Lambda function to respond to the policy which is invoked when the burst credit buffer falls below 20%. Navigate to the [Policies](https://app.metricly.com/#/alerts) page. We're going to create our Webhook on the **_AWS EBS -- Depleted Burst Balance and High IOPS Utilization_** policy. Locate this policy and click on it.
...
You'll need a name and the URL created for the API. Select the "custom payload" option. I entered the following payload which will allow my volume to be scaled to 16 GiB in 1 GiB increments. **_${elementId}_** allows the volumeId to be populated by Metricly, which is especially useful if you are applying a policy like this to multiple volumes.
