...
```json
    ...
    "total":{
        "results":[
            {
                "result":2.3521767464428818
        ],
        "beginTimeSeconds":0,
        "endTimeSeconds":0,
        "inspectedCount":48916
    },
    "timeSeries":[
        {
            "results":[
                {
                "result":2.0489708992322457
            ],
            "beginTimeSeconds":1522337023,
            "endTimeSeconds":1522337083,
            "inspectedCount":1042
        },
        {
            "results":[
                {
                "result":2.168361755670103
            ],
            "beginTimeSeconds":1522337083,
            "endTimeSeconds":1522337143,
            "inspectedCount":970
    },
    ...
```
#### Your Metricly Account and What It Can Do
Metricly is different from other APM and monitoring tools available in this space. Not only does Metricly provide a comprehensive monitoring and analytics platform, but they have also invested heavily in providing tools to leverage your data and provide you with [dashboards](https://www.metricly.com/product/dashboards-and-reports), [anomaly detection](https://www.metricly.com/what-is-anomaly-detection) and [policies](https://www.metricly.com/support/events/policies) designed to alert you to problems before they become critical.
...
#### Importing the Data
The first step is to get an API Key. You can find this by logging in to your Metricly account and navigating to the [Integrations](https://app.metricly.com/#/integrations) page. The integration we’ll be using is the **Custom Integration**. This integration will allow us to submit custom metrics through the Ingest API.  You’ll want to enable the API Key if it isn’t enabled and copy the provided URL into your REST client.

![img](https://www.metricly.com/wp-content/uploads/2018/04/Fig.-2-Custom-Datasource-Integration-300x108.png)
<!-- Custom Datasource Integration -->

![img](https://www.metricly.com/wp-content/uploads/2018/04/Fig.-3-Finding-and-Enabling-Your-API-Key.png)
<!-- POST Request with Data -->

I’ll be using Postman to submit my metrics to the API, but you could use any tool capable of sending POST requests. The URL is **_https://api.app.metricly.com/ingest/{apiId}_**  with your unique API Key replacing **_{apiId}_**.

![img](https://www.metricly.com/wp-content/uploads/2018/04/Fig.-4-POST-Request-with-Data.png)
<!-- POST Request with Data -->

The request requires basic authorization, which will require your Metricly username and password. You will also need to set the **Content-Type** header to **_application/json_**.

You will also need to work with the format of your data to put it into a form which Metricly can ingest. You can find details about the composition of the request (including optional and required parameters) on the [Metricly API Metrics Support page – Create a Data Sample](https://www.metricly.com/support/api/metrics#create-a-data-sample).
...
[
  {
    "id": "my_Test_Web_Service",
    "name": "My Test Web Service",
    "type": "server",
    "metrics":[
      {
        "id":"service.responseTime",
        "name":"Response Time (ms)"
      }
    ],
    "samples": [
      {
        "metricId":"service.responseTime",
        "val":2.0489708992322457,
        "timestamp":1522337023000
      },
      {
        "metricId":"service.responseTime",
        "val":2.168361755670103,
        "timestamp":1522337083000
      },
...
```
...
Our first step will be to create a new dashboard for our service and add a new metric widget to the dashboard to display the data. Log in to your Metricly account and navigate to the [Dashboards page](https://app.metricly.com/#/dashboards/management). Add a new dashboard by clicking the **New Dashboard** button.

![img](https://www.metricly.com/wp-content/uploads/2018/04/New-Dashboard.png)
<!-- New Dashboard -->
...
![img](https://www.metricly.com/wp-content/uploads/2018/04/Add-Widget.png)
<!-- Add Widget -->
...
![img](https://www.metricly.com/wp-content/uploads/2018/04/Fig.-6-Adding-the-Single-Metric-Widget-to-the-Dashboard.png)
<!-- Adding the Single Metric Widget to the Dashboard -->

We’ll make this a **Time Series** widget. Select **_service.responseTime_** from the metric drop-down, choose **_milliseconds (ms)_** as the display unit, and choose a name for the widget. Click **Save** and your widget will appear on your dashboard.

![img](https://www.metricly.com/wp-content/uploads/2018/04/Fig.-7-Configure-the-Time-Series-Widget.png)
...
![img](https://www.metricly.com/wp-content/uploads/2018/04/Fig.-8-Response-Time-Shown-as-Time-Series-on-Dashboard.png)
...
Navigate to the [Alerts page](https://app.metricly.com/#/alerts) within your Metricly account. The Alerts page automatically includes alerts based on the integrations which you have enabled on your account. Because we’re working with custom metrics in this example, we’ll need to create our own. Begin by clicking on the **New Policy** button.

![img](https://www.metricly.com/wp-content/uploads/2018/04/New-Policy.png)
<!-- New Policy -->
...
![img](https://www.metricly.com/wp-content/uploads/2018/04/Fig.-9-Defining-the-Name-Type-and-Scope-of-the-Policy.png)
...
We’ll set the metric to **_service.responseTime_** and set a **Static Threshold** of **_More Than 50_**. You’ll notice other deviation options available. These different options allow you to set your policy based on a deviation of a certain percentage, to compare metrics to each other, or set static thresholds like we’re doing with this one.

![img](https://www.metricly.com/wp-content/uploads/2018/04/Fig.-10-Defining-Conditions-for-which-This-Policy-will-Trigger.png)
<!-- Setting up Notification Channels for When the Policy is Triggered -->
...
![img](https://www.metricly.com/wp-content/uploads/2018/04/Fig.-11-Setting-up-Notification-Channels-for-When-the-Policy-is-Triggered.png)
<!-- Setting up Notification Channels for When the Policy is Triggered -->
...
If you would like additional information about how to get started with your Metricly account or additional features available within your account, I recommend the [Metricly Support Portal](https://www.metricly.com/support).
