...
Metricly has a [customized dashboard](/netuitive-dashboard-upgrades) dedicated to showing data collected from your Linux server on EC2. In this dashboard, you can monitor the data sources outlined above---network performance, CPU, and so on. You can also create customized metrics not available in Metricly by default.
...
Now that we've gotten an overview of [monitoring a server on AWS](/getting-started-netuitive-aws), let's take a look at some specific metrics you can collect and monitor in Metricly to help get the most performance out of your Linux servers.
